...
export const licenses = mysqlTable("licenses", {
  ...
  status: mysqlEnum("status", ["active", "trial", "paused", "expired", "revoked"]).notNull().default("trial"),
...
export const licenseDevices = mysqlTable("licenseDevices", {
  id: int("id").autoincrement().primaryKey(),
...
export type InsertLicenseDevice = typeof licenseDevices.$inferInsert;
...
export function planFromId(id: string) {
  return planCatalog.find((plan) => plan.id === id) ?? planCatalog[1];
}
